...
function validateAdditionItems(items: any): string | null {
  ...
  for (const item of items) {
    ...
    if (
      item.ref_id === undefined ||
      item.ref_id === null ||
      String(item.ref_id).trim() === ""
    ) {
...
if (IS_PRODUCTION) {
  ...
  if (JWT_SECRET.length < 32) {
    console.error(
      "[ERR] JWT_SECRET terlalu lemah untuk production (min. 32 karakter acak). " +
...
if (!IS_PRODUCTION) {
  console.log(
    "[KEY] SERVER KEY len:",
    MIDTRANS_SERVER_KEY.length,
    "| last char code:",
    MIDTRANS_SERVER_KEY.charCodeAt(MIDTRANS_SERVER_KEY.length - 1),
  );
  console.log(
    "[KEY] CLIENT KEY len:",
    MIDTRANS_CLIENT_KEY.length,
    "| last char code:",
    MIDTRANS_CLIENT_KEY.charCodeAt(MIDTRANS_CLIENT_KEY.length - 1),
  );
}
...
(function validateMidtransConfig() {
  if (!MIDTRANS_SERVER_KEY || !MIDTRANS_CLIENT_KEY) {
    console.warn(
      "[WARN] MIDTRANS_SERVER_KEY atau MIDTRANS_CLIENT_KEY belum diisi di .env",
    );
  ...
  const serverEnv = serverIsSandbox
    ? "sandbox"
    : serverIsProduction
      ? "production"
      : "unknown";
  const clientEnv = clientIsSandbox
    ? "sandbox"
    : clientIsProduction
      ? "production"
      : "unknown";
  ...
  if (
    ...
    console.warn(
      "[WARN] Server key dan Client key terdeteksi beda environment!",
    );
    console.warn(
      `    Server key: ${serverEnv} (${MIDTRANS_SERVER_KEY.substring(0, 20)}...)`,
    );
    console.warn(
      `    Client key: ${clientEnv} (${MIDTRANS_CLIENT_KEY.substring(0, 20)}...)`,
    );
  }
  ...
  if (MIDTRANS_IS_PRODUCTION && serverIsSandbox) {
    console.warn(
      "[WARN] MIDTRANS_IS_PRODUCTION=true tapi key berawalan SB- (sandbox).",
    );
  }
  if (!MIDTRANS_IS_PRODUCTION && serverIsProduction) {
    console.warn(
      `[WARN] MIDTRANS_IS_PRODUCTION=false dengan key berawalan Mid-server- (production-style).`,
    );
    console.warn(
      "    Jika ini akun sandbox tanpa prefix SB-, abaikan pesan ini.",
    );
  }

  console.log(
    `[OK] Midtrans configured: ${MIDTRANS_IS_PRODUCTION ? "PRODUCTION (real money!)" : "SANDBOX (test mode)"}`,
  );
})();
...
async function testDB() {
  ...
  const productColSet = new Set(
    (productCols as any[]).map((r) => r.COLUMN_NAME),
  );
  ...
  if (!productColSet.has("image_mime")) {
    await pool.query(
      "ALTER TABLE products ADD COLUMN image_mime VARCHAR(50) DEFAULT 'image/webp'",
    );
    console.log("  -> migrated: products.image_mime added");
  }
  if (!productColSet.has("slug")) {
    await pool.query(
      "ALTER TABLE products ADD COLUMN slug VARCHAR(200) DEFAULT NULL, ADD INDEX idx_slug (slug)",
    );
  ...
  if (teamColSet.size > 0 && !teamColSet.has("image_mime")) {
    await pool.query(
      "ALTER TABLE team ADD COLUMN image_mime VARCHAR(50) DEFAULT 'image/webp'",
    );
  ...
  const ordInvColSet = new Set(
    (ordInvCols as any[]).map((r: any) => r.COLUMN_NAME),
  );
  if (!ordInvColSet.has("invoice_number")) {
    await pool.query(
      "ALTER TABLE orders ADD COLUMN invoice_number VARCHAR(30) DEFAULT NULL",
    );
    console.log("  -> migrated: orders.invoice_number added");
  }
  if (!ordInvColSet.has("invoice_token")) {
    await pool.query(
      "ALTER TABLE orders ADD COLUMN invoice_token VARCHAR(64) DEFAULT NULL",
    );
    console.log("  -> migrated: orders.invoice_token added");
  }
  if (!ordInvColSet.has("invoice_sent_at")) {
    await pool.query(
      "ALTER TABLE orders ADD COLUMN invoice_sent_at TIMESTAMP NULL DEFAULT NULL",
    );
  ...
  if (!oaColSet.has("midtrans_order_id")) {
    await pool.query(
      "ALTER TABLE order_additions ADD COLUMN midtrans_order_id VARCHAR(100) DEFAULT NULL",
    );
    await pool.query(
      "ALTER TABLE order_additions ADD INDEX idx_oa_midtrans (midtrans_order_id)",
    );
...
async function startServer() {
  ...
  // Gzip semua response teks (JS/CSS/HTML/JSON). Tanpa ini bundle dikirim mentah.
  app.use(compression());
    ...
    cors({
      origin: (origin, callback) => {
        ...
        if (!isProduction) {
          ...
          const isPrivate =
            /^http:\/\/(192\.168\.|172\.(1[6-9]|2\d|3[01])\.|10\.)\d+\.\d+(:\d+)?$/.test(
              origin,
            );
  ...
  app.post("/api/auth/register", authLimiter, async (req, res) => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, error: "Username harus 3â€“50 karakter" });
    }
    ...
    try {
      ...
      const token = jwt.sign(
        { id: ins.insertId, username, role: "user" },
        JWT_SECRET,
        { expiresIn: "24h" },
      );
      ...
      res
        .status(500)
        .json({ success: false, error: "Gagal mendaftar, coba lagi" });
    }
  });
  ...
  app.post("/api/auth/login", authLimiter, async (req, res) => {
    ...
    try {
      ...
      const dummyHash =
        "$2b$12$invalidhashfortimingggggggggggggggggggggggggggggg";
      const isValid = user
  ...
  app.get("/api/services", async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Gagal mengambil layanan" });
    }
  });
    ...
    try {
      ...
      res.json({
        ...
        data: (rows as any[]).map((row) => ({
          ...
          image_alt:
            row.image_alt ||
            generateProductAlt(row.name, row.brand, row.type, row.capacity),
        })),
      });
    } catch (err) {
  ...
  app.get("/api/materials", async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Gagal mengambil material" });
    }
  });
  ...
  app.get("/api/team", async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Gagal mengambil data tim" });
    }
  });
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengambil produk" });
      }
    },
  );
  ...
  app.post(
    ...
    upload.single("image"),
    async (req, res) => {
      const { name, brand, type, capacity, price, description, image_alt } =
        req.body;
      try {
        // Auto-optimize gambar ke WebP
        const optimized = await optimizeImage(req.file?.buffer);
        const altText =
          image_alt?.trim() || generateProductAlt(name, brand, type, capacity);
        const slug =
          slugify(`${name} ${brand} ${capacity}`.trim()) || slugify(name);
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal menambah produk" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengupdate produk" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (!name || !price)
        return res
          .status(400)
          .json({ success: false, error: "Nama dan harga wajib diisi" });
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal menambah layanan" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengupdate layanan" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal menghapus layanan" });
      }
    },
  );
  ...
  app.get("/api/service-tiers/:serviceId", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Gagal mengambil tier harga" });
    }
  });

  // Service Tiers (admin CRUD)
  app.get(
    "/api/admin/service-tiers/:serviceId",
    authenticateToken,
    requireAdmin,
    async (req, res) => {
      try {
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengupdate tier" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        await pool.query("DELETE FROM service_tiers WHERE id=?", [
          req.params.id,
        ]);
        res.json({ success: true });
      } catch {
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal menambah anggota tim" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req: any, res) => {
      const { id } = req.params;
      const { name, position, role_label, bio, phone, sort_order, is_active } =
        req.body;
      try {
        if (req.file?.buffer) {
          const optimized = await optimizeImage(req.file.buffer);
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengupdate anggota tim" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal menghapus anggota tim" });
      }
    },
  );
      ...
      try {
        ...
        for (const it of cartItems) {
          ...
          if (itemType === "service") {
            ...
            if (tier) {
              ...
              if (!t.length)
                return res
                  .status(400)
                  .json({ success: false, error: "Tier jasa tidak ditemukan" });
              ...
              if (!s.length)
                return res
                  .status(400)
                  .json({ success: false, error: "Jasa tidak ditemukan" });
          ...
          if (!Number.isFinite(unitPrice) || unitPrice < 0) {
            return res
              .status(400)
              .json({ success: false, error: "Harga item tidak valid" });
          }
          itemDetails.push({
        ...
        if (!totalPrice || totalPrice <= 0) {
          return res
            .status(400)
            .json({ success: false, error: "Total harga tidak valid" });
        }
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal membuat token pembayaran" });
      }
    },
  );
  ...
  async function ensureAdditionInvoice(additionId: number | string) {
    return withInvoiceLock(async (conn) => {
      const [rows]: any = await conn.query(
        "SELECT invoice_number FROM order_additions WHERE id=?",
        [additionId],
      );
      if (!rows.length) return null;
    ...
    try {
      ...
      if (orderId.startsWith("ADD-")) {
        ...
        const [byMid]: any = await pool.query(
          "SELECT id FROM order_additions WHERE midtrans_order_id=?",
          [orderId],
        );
        if (byMid.length) {
        ...
        if (additionId === null) {
          return res
            .status(404)
            .json({ success: false, error: "Addition tidak ditemukan" });
        }
        if (
          ["capture", "settlement"].includes(notification.transaction_status)
        ) {
          await pool.query(
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        if (
          req.user.role !== "admin" &&
          String(orderRows[0].user_id) !== String(req.user.id)
        ) {
          return res
            .status(404)
            .json({ success: false, error: "Order tidak ditemukan" });
        ...
        res.json({
          success: true,
          data: synced || { status: "pending", source: "database" },
        });
      } catch {
        res
          .status(500)
    ...
    async (req: any, res) => {
      try {
        ...
        res.json({
          success: true,
          data: {
            midtransStatus: synced.status,
            updatedStatus: synced,
            source: "midtrans_api",
          },
        });
      } catch {
  ...
  app.post("/api/orders", authenticateToken, async (req: any, res) => {
    const { quantity, productId, customerName, phone, address, paymentMethod } =
      req.body;
    const user = req.user;
    ...
    async (_req, res) => {
      try {
        ...
        const weeklyRevenue = Array.from({ length: 12 }, (_, i) => {
          ...
          return makePeriod(
            `${isoDate(startDate)}_${isoDate(endDate)}`,
        ...
        const monthlyIndex = new Map(
          monthlyRevenue.map((item, index) => [item.month, index]),
        );
        const yearlyIndex = new Map(
          yearlyRevenue.map((item, index) => [item.month, index]),
        );
        ...
        for (const row of periodOrders as any[]) {
          ...
          const diffDays = Math.floor(
            (todayStart.getTime() - orderDay.getTime()) / dayMs,
          );
        ...
        res.json({
          ...
          data: {
            ...
            topTeknisi: topTeknisi.map((r: any) => ({
              id: r.id,
              name: r.name,
              assigned: Number(r.total_assigned) || 0,
              completed: Number(r.completed_orders) || 0,
            })),
            financialBreakdown: financialBreakdown.map((r: any) => ({
              status: r.status,
              orderCount: Number(r.order_count) || 0,
              totalAmount: Number(r.total_amount) || 0,
            })),
            orderStatusBreakdown: orderStatusBreakdown.map((r: any) => ({
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengambil laporan" });
      }
    },
  );
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengambil users" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengupdate user" });
      }
    },
  );
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Gagal mengambil data teknisi" });
      }
    },
  );
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        res.json({
          success: true,
          data: { payment_status: "settlement", order_status: nextOrderStatus },
        });
      } catch (err) {
        console.error("Manual verify payment error:", err);
        res
          .status(500)
          .json({ success: false, error: "Gagal verifikasi pembayaran" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      if (!file)
        return res
          .status(400)
          .json({ success: false, error: "Tidak ada foto yang diunggah" });
      if (!["before", "after"].includes(type))
        return res
          .status(400)
          .json({ success: false, error: "Tipe foto tidak valid" });
      try {
        // Cegah IDOR: teknisi hanya boleh upload ke order miliknya (admin bebas)
        const [ownRows]: any = await pool.query(
          "SELECT teknisi_id FROM orders WHERE id=?",
          [req.params.id],
        );
        if (!ownRows.length)
          return res
        ...
        if (
          req.user.role !== "admin" &&
          String(ownRows[0].teknisi_id) !== String(req.user.id)
        ) {
          return res
            .status(403)
            .json({ success: false, error: "Order ini bukan milik Anda" });
        }
        ...
        await pool.query(
          ...
          [
            ...
            optimized ? "image/webp" : file.mimetype,
          ],
        );
        res.json({ success: true });
      } catch {
      ...
      try {
        ...
        if (!allowed) {
          return res
            ...
            .json({
              success: false,
              error: "Tidak diizinkan mengakses foto ini",
            });
        }
  ...
  if (isProduction) {
    ...
    // /assets/* punya hash di nama file → aman di-cache 1 tahun.
  ...
  app.get("/sitemap.xml", async (_req, res) => {
    try {
      ...
      for (const p of products as any[]) {
        ...
        const lastMod = p.last_mod
          ? new Date(p.last_mod).toISOString().split("T")[0]
          : today;
        xml += `  <url>\n`;
        ...
        if (p.has_image) {
          const altText =
            p.image_alt ||
            generateProductAlt(p.name, p.brand, p.type, p.capacity);
      ...
      const itemList = {
        ...
        itemListElement: (products as any[]).map((p, i) => ({
          ...
          item: {
            ...
            description:
              p.description ||
              `${p.name} ${p.brand || ""} ${p.capacity || ""} - tersedia di HDB Airconds Mojokerto`.trim(),
            ...(p.has_image
              ? { image: `${SITE_URL}/api/products/${p.id}/image` }
              : {}),
            offers: {
              "@type": "Offer",
  ...
  app.get("/api/material-catalog", authenticateToken, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Gagal mengambil katalog" });
    }
  });
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      if (!name || !unit || !price) {
        return res
          .status(400)
          .json({ success: false, message: "name, unit, price wajib diisi" });
      }
      try {
        const [result]: any = await pool.query(
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal menambah item" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal mengubah item" });
      }
    },
  );
  ...
  app.patch(
    "/api/material-catalog/:id/toggle",
    authenticateToken,
    requireAdmin,
    async (req: any, res) => {
      try {
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        const [orderRows]: any = await conn.query(
          "SELECT id, user_id, teknisi_id FROM orders WHERE id=?",
          [orderId],
        );
        if (!orderRows.length) {
          await conn.rollback();
        ...
        const isOwner =
          ord.user_id != null && String(ord.user_id) === String(req.user.id);
        const isAssignedTeknisi =
          req.user.role === "teknisi" &&
          String(ord.teknisi_id) === String(req.user.id);
        if (req.user.role !== "admin" && !isAssignedTeknisi && !isOwner) {
          ...
          return res
            ...
            .json({
              success: false,
              message: "Tidak diizinkan menambah item ke order ini",
            });
        }
          ...
          if (item.item_type === "material") {
            ...
            if (!mRows.length) {
              ...
              return res
                ...
                .json({
                  success: false,
                  message: `Material id ${item.ref_id} tidak ditemukan`,
                });
            }
            const m = mRows[0];
            resolvedItems.push({
            ...
            if (!sRows.length) {
              ...
              return res
                ...
                .json({
                  success: false,
                  message: `Service id ${item.ref_id} tidak ditemukan`,
                });
            }
            const s = sRows[0];
            resolvedItems.push({
        ...
        const [addResult]: any = await conn.query(
         ...
         VALUES (?, ?, ?, 'pending_admin')`,
          [
            orderId,
            req.user.role === "teknisi" ? "teknisi" : "customer",
            req.user.id,
          ],
        );
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal membuat pengajuan" });
      } finally {
        conn.release();
      }
    },
    ...
    async (req: any, res) => {
      try {
        ...
        const [allItems]: any = await pool.query(
          "SELECT * FROM order_addition_items WHERE order_addition_id IN (?)",
          [additionIds],
        );
        ...
        const result = rows.map((row: any) => {
          ...
          const total = items.reduce(
            (s: number, i: any) => s + Number(i.subtotal),
            0,
          );
  ...
  app.patch(
    "/api/order-additions/:id/admin-review",
    authenticateToken,
    requireAdmin,
    async (req: any, res) => {
      ...
      try {
        const [rows]: any = await pool.query(
          "SELECT * FROM order_additions WHERE id=?",
          [req.params.id],
        );
        if (!rows.length)
          return res
        ...
        if (rows[0].status !== "pending_admin") {
          return res
            .status(400)
            .json({ success: false, message: "Status bukan pending_admin" });
        }
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal memproses review" });
      }
    },
  );
  ...
  app.get("/api/order-additions/token/:token", async (req, res) => {
    try {
      ...
      if (!rows.length)
        return res
          .status(404)
          .json({ success: false, message: "Link tidak valid" });
      const addition = rows[0];
      const [items]: any = await pool.query(
        "SELECT * FROM order_addition_items WHERE order_addition_id=?",
        [addition.id],
  ...
  app.patch(
    "/api/order-additions/:id/customer-response",
      ...
      try {
        ...
        if (!payment_method) {
          return res
            ...
            .json({
              success: false,
              message: "payment_method wajib diisi saat approve",
            });
        }
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal memproses respon" });
      }
    },
  );
  ...
  app.post(
    "/api/order-additions/:id/payment",
    customerPaymentLimiter,
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal membuat transaksi" });
      }
    },
  );
  ...
  app.post(
    "/api/order-additions/:id/confirm-payment",
    customerPaymentLimiter,
    async (req, res) => {
      const { token, midtransOrderId } = req.body as {
        token: string;
      ...
      try {
        ...
        if (
          !midtransOrderId ||
          !String(midtransOrderId).startsWith(`ADD-${add.id}-`)
        ) {
          return res
            .status(400)
            .json({ success: false, message: "midtransOrderId tidak valid" });
        }
        ...
        return res.json({
          success: true,
          status: "pending",
          midtransStatus: st.transaction_status,
        });
      } catch (e) {
        res
          .status(500)
          .json({ success: false, message: "Gagal verifikasi pembayaran" });
      }
    },
  );
      ...
      try {
        ...
        if (!["customer_rejected", "admin_rejected"].includes(rows[0].status)) {
          ...
          return res
            ...
            .json({
              success: false,
              message: "Hanya bisa revisi setelah ditolak",
            });
        }
        // Hapus items lama, insert baru
        await conn.query(
          "DELETE FROM order_addition_items WHERE order_addition_id=?",
          [req.params.id],
        );
        for (const item of items) {
          ...
          if (item.item_type === "material") {
            ...
            if (!iRows.length) {
              ...
              return res
                ...
                .json({
                  success: false,
                  message: `Material ${item.ref_id} tidak ditemukan`,
                });
            }
            iName = iRows[0].name;
            iUnit = iRows[0].unit;
            iPrice = Number(iRows[0].price);
          } else {
            const [iRows]: any = await conn.query(
              "SELECT name, price FROM services WHERE id=?",
              [item.ref_id],
            ...
            if (!iRows.length) {
              ...
              return res
                ...
                .json({
                  success: false,
                  message: `Jasa ${item.ref_id} tidak ditemukan`,
                });
            }
            iName = iRows[0].name;
            iUnit = "unit";
          ...
          await conn.query(
           ...
           VALUES (?,?,?,?,?,?,?,?)`,
            [
              ...
              item.quantity,
              iPrice,
              iPrice * item.quantity,
            ],
          );
        }
        await conn.query(
          `UPDATE order_additions SET status='pending_admin', updated_at=NOW() WHERE id=?`,
          [req.params.id],
        );
        await conn.commit();
  ...
  app.patch(
    "/api/order-additions/:id/escalate",
    authenticateToken,
    requireTeknisi,
    async (req: any, res) => {
      try {
  ...
  app.get(
    "/api/order-additions/token/:token/invoice-data",
    async (req, res) => {
      try {
        const [rows]: any = await pool.query(
          `SELECT oa.*, o.customer_name, o.phone as customer_phone, o.address as customer_address,
        ...
        const addTotal = addItems.reduce(
          (s: number, i: any) => s + Number(i.subtotal),
          0,
        );
        res.json({
  ...
  app.post(
    "/api/order-additions/:id/send-invoice",
    authenticateToken,
    requireAdmin,
    async (req: any, res) => {
      try {
        ...
        if (!rows.length)
          return res
            ...
            .json({
              success: false,
              message: "Data tidak ditemukan atau belum paid",
            });
        const add = rows[0];
        ...
        const invoiceNumber =
          add.invoice_number || (await ensureAdditionInvoice(req.params.id));
        if (!invoiceNumber)
          return res
            .status(500)
            .json({ success: false, message: "Gagal membuat nomor invoice" });
        ...
        res
          .status(500)
          .json({ success: false, message: "Gagal mengirim invoice" });
      }
    },
  );
